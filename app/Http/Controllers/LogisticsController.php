...
class LogisticsController extends Controller
{
    ...
    public function send_email($logistic_id, $status)
    {
        ...
        if (count($cc)) {
            foreach ($cc as $rcpt) {
                if($this->validateEmail(trim($rcpt->email))){
                    array_push($cc_recipients,trim($rcpt->email));        
                }
            }
        }
            ...
            foreach ($emails as $email) {
                if (strlen($email) > 2) {
                    if($this->validateEmail(trim($email))){
                        array_push($recipients,trim($email));      
                    }
        ...
        logger($recipients);
        logger($cc_recipients);
        Mail::to($recipients)
        ->cc($cc_recipients)
        ->send(new PoUpdateMail($logistic_id, $status));

        //Mail::to('jatano@philsaga.com')->send(new PoUpdateMail($logistic_id, $status));
    }

    // send email
    public function send_email_old($logistic_id, $status)
    {
        logger($logistic_id);
        return true;
        $l = logistics::where('id', $logistic_id)->first();
        $p = PO::where('id', $l->poId)->first();
        // $w = PO::where('id', $l->poId)->first();
