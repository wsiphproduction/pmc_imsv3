...
class McdController extends Controller
{
    ...
    public function pendings()
    {
        ...
        //     ->get();
        $rolesPermissions = $this->roleRightService->hasPermissions("Pending Waybills");
        ...
        $datas = Logistics::whereExists(function ($query) {
            $query->select(DB::raw(1))->from('po')->whereRaw('po.id = logistics.poId')->where('po.status', 'OPEN');
        })
            ->where('status', 'Delivered')
            ->orderBy('actualDeliveryDate', 'asc')
            ->get();
    ...
    public function drr_store(Request $req)
    {
        ...
        $notification = array(
            'message' => 'Delivery Receiving Report has been added.',
            'alert-type' => 'info'
        );
    ...
    public function closePOifDelivered($id)
    {
        ...
        if ($balance == 0) {
            ...
            $this->ClosePOifCompleted($id);
        }
    }

    public function ClosePOifCompleted($id)
    {
        $po = PO::whereId($id)->first();
        if ($po->paymentStatus == 'PAID' && $po->deliveryStatus == 'DELIVERED') {
            $closePo = PO::whereId($id)->update([
                'status' => 'CLOSED',
    ...
    {
        $rolesPermissions = $this->roleRightService->hasPermissions("Receiving DRR Summary");
