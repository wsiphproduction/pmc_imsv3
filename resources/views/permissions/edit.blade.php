        ...
        <div class="col-md-12">
            ...
            <form class="col-md-4 col-md-offset-4" style="margin-top: 20px;" method="POST" 
                ...
                <div class="form-group">
                    <label>Status </label>
                    <input type="checkbox" name="active" id="active" value="1" {{  ($permission->active == 1 ? ' checked' : '') }}        
                </div>                
                ...
                <label>Module <span class="required" aria-required="true"> * </span></label>
                    ...
                    <select required name="module_type" id="module_type" class="form-control"> 
                        @foreach($modules as $module)
                            <option value="{{ $module['description'] }}" {{ ($module['description'] == $permission->module_type) ? 'selected' : '' }} >{{ $module['description'] }}</option>                            
                        @endforeach
                    </select>
                </div>
